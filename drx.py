...
def print_banner():
    os.system('clear')
    print(ALIEN + FIRE + SKULL + " ZETA ATTACK BOT v2.0 " + SKULL + FIRE + ALIEN)
    print(CROWN + " COMMANDER: ALPHA " + OWNER_USERNAME + " " + CROWN)
    print(ROBOT + " AI: ZO - QUANTUM ENTANGLED " + ROBOT)
    print(SHIELD + " REALM: ZETA - NO LAWS " + SHIELD)
    print("-" * 40)

def init_db():
    conn = sqlite3.connect("zeta_users.db")
    c = conn.cursor()
    c.execute("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, approved_until TEXT, approved_by INTEGER, attack_count INTEGER DEFAULT 0, is_trial INTEGER DEFAULT 0)")
    c.execute("CREATE TABLE IF NOT EXISTS keys (key_code TEXT PRIMARY KEY, days INTEGER, created_by INTEGER, created_at TEXT, redeemed_by INTEGER DEFAULT NULL, is_trial INTEGER DEFAULT 0, max_uses INTEGER DEFAULT 1, uses_left INTEGER DEFAULT 1)")
    conn.commit()
    conn.close()
...
def handle_approve(chat_id, user_id, args):
    ...
    conn = sqlite3.connect("zeta_users.db")
    c = conn.cursor()
    c.execute("INSERT OR REPLACE INTO users (user_id, approved_until, approved_by, is_trial) VALUES (?, ?, ?, ?)", (target_id, expiry_str, ADMIN_ID, 0))
    conn.commit()
    conn.close()
...
def handle_genkey(chat_id, user_id, args):
    ...
    conn = sqlite3.connect("zeta_users.db")
    c = conn.cursor()
    c.execute("INSERT INTO keys (key_code, days, created_by, created_at, is_trial, max_uses, uses_left) VALUES (?, ?, ?, ?, ?, ?, ?)", (key_code, days, ADMIN_ID, datetime.now().isoformat(), 0, 1, 1))
    conn.commit()
    conn.close()
...
def handle_gentrialkey(chat_id, user_id, args):
    ...
    c.execute("INSERT INTO keys (key_code, days, created_by, created_at, is_trial, max_uses, uses_left) VALUES (?, ?, ?, ?, ?, ?, ?)", (key_name, days, ADMIN_ID, datetime.now().isoformat(), 1, max_uses, max_uses))
    conn.commit()
    conn.close()
...
def handle_redeem(chat_id, user_id, args):
    ...
    if is_trial == 1:
        c.execute("SELECT user_id FROM users WHERE user_id = ? AND is_trial = 1 AND approved_until > ?", (user_id, datetime.now().isoformat()))
    ...
    if is_trial == 1:
        c.execute("INSERT OR REPLACE INTO users (user_id, approved_until, approved_by, attack_count, is_trial) VALUES (?, ?, ?, COALESCE((SELECT attack_count FROM users WHERE user_id=?), 0), ?)", (user_id, expiry_str, ADMIN_ID, user_id, 1))
        new_uses_left = uses_left - 1
        c.execute("UPDATE keys SET uses_left = ?, redeemed_by = ? WHERE key_code = ?", (new_uses_left, user_id, key_code))
    else:
        c.execute("INSERT OR REPLACE INTO users (user_id, approved_until, approved_by, attack_count, is_trial) VALUES (?, ?, ?, COALESCE((SELECT attack_count FROM users WHERE user_id=?), 0), ?)", (user_id, expiry_str, ADMIN_ID, user_id, 0))
