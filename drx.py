...
# ================= CONFIG - EDIT THESE =================
BOT_TOKEN = "8434680550:AAFtO13ftn95TIRMR9uqSGS5j7NazNghSdE"  # <-- REPLACE WITH YOUR BOT TOKEN
ADMIN_ID = 6465928598  # <-- REPLACE WITH YOUR TELEGRAM USER ID
# =======================================================
...
def handle_gentrialkey(chat_id, user_id, args):
    ...
    if c.fetchone():
        send_message(chat_id, f"{EMOJI['cross']} Key `{key_name}` already exists.", parse_mode="Markdown")
        conn.close()
        return
...
def handle_redeem(chat_id, user_id, args):
    ...
    if is_trial == 1:
        ...
        if existing_trial:
            send_message(chat_id, f"{EMOJI['cross']} You already have an active trial!", parse_mode="Markdown")
            conn.close()
            return
    ...
    if is_trial == 1:
        ...
        msg = f"{EMOJI['check']}✅ TRIAL ACCESS GRANTED ✅{EMOJI['check']}\n\n🔑 Key: `{key_code}`\n👤 You now have trial access for {int(days)} days!\n📊 Uses remaining: {uses_remaining}{uses_text}\n\n💀 Type `/attack` to begin your mayhem."
    else:
        msg = f"{EMOJI['check']}✅ ACCESS GRANTED ✅{EMOJI['check']}\n\n🔑 Key redeemed: `{key_code}`\n\n👤 You now have Zeta access for {int(days)} days!\n\n💀 Type `/atta
