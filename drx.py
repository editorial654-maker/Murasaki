...
def handle_gentrialkey(chat_id, user_id, args):
    ...
    if len(args) != 3:
        send_message(chat_id, f"{EMOJI['warning']} Usage: /gentrialkey <key> <duration> <max_uses>\n\n"
                     f"Duration format: <number><h/d/m>\n"
                     f"Examples:\n"
                     f"`/gentrialkey TRIAL1 24h 5` - 24 hours, max 5 uses\n"
                     f"`/gentrialkey TRIAL2 7d 10` - 7 days, max 10 uses\n"
                     f"`/gentrialkey TRIAL3 1m 1` - 1 month, max 1 use",
                     parse_mode="Markdown")
        return
...
def handle_redeem(chat_id, user_id, args):
    ...
    if is_trial == 1:
        ...
        uses_text = f"\n\n⚠️ This key can be used {uses_remaining} more time(s)!" if uses_remaining > 0 else f"\n\n⚠️ This key has been fully used and will expire."
