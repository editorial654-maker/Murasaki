#!/usr/bin/env python3
# Zo - Telegram Attack Bot for Zeta Realm (ANIMATED + EMOJIS - FIXED + TRIAL KEYS + BROADCAST)
...
ADMIN_ID = 6465928598  # This ID belongs to @YOWAI_MO_456
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS keys (
        ...
        is_trial INTEGER DEFAULT 0,
        trial_duration_hours INTEGER DEFAULT NULL,
...
def handle_help(chat_id, user_id):
    if not is_approved(user_id):
        ...
        return
    msg = f"""📖 {EMOJI['alien']} ZETA ATTACK BOT v3.0 {EMOJI['alien']} 📖
...
def handle_genkey(chat_id, user_id, args):
    ...
    conn = sqlite3.connect("zeta_users.db")
    c = conn.cursor()
    c.execute("INSERT INTO keys (key_code, days, created_by, created_at, is_trial, max_uses, uses_remaining) VALUES (?, ?, ?, ?, ?, ?, ?)",
              (key_code, days, ADMIN_ID, datetime.now().isoformat(), 0, 1, 1))
    conn.commit()
    conn.close()
    
    # Send copiable key only
...
def handle_gentrialkey(chat_id, user_id, args):
    ...
    elif duration_str.endswith('months'):
        hours = int(duration_str.replace('months', '')) * 30 * 24
    elif duration_str.endswith('month'):
        hours = int(duration_str.replace('month', '')) * 30 * 24
    elif duration_str.endswith('year'):
        hours = int(duration_str.replace('year', '')) * 365 * 24
    else:
...
def handle_redeem(chat_id, user_id, args):
    ...
    # Check premium keys first
    conn = sqlite3.connect("zeta_users.db")
    c = conn.cursor()
    c.execute("SELECT days, redeemed_by FROM keys WHERE key_code = ? AND is_trial = 0", (key_code,))
    row = c.fetchone()
