#!/usr/bin/env python3
# Zo - Telegram Attack Bot for Zeta Realm (NO DUPLICATE REDEEM + FULL EMOJIS)
...
def print_banner():
    ...
    os.system('clear')
    banner = f"""
{EMOJI['alien']}{EMOJI['fire']}{EMOJI['skull']} ZETA ATTACK BOT v3.1 {EMOJI['skull']}{EMOJI['fire']}{EMOJI['alien']}
...
def init_db():
    ...
    # NEW TABLE: Track which user redeemed which trial key
...
def handle_help(chat_id, user_id):
    if not is_approved(user_id):
        ...
        return
    msg = f"""📖 {EMOJI['alien']} ZETA ATTACK BOT v3.1 {EMOJI['alien']} 📖
...
{EMOJI['check']} /yes - Confirm blocked port
{EMOJI['key']} /redeem <key> - Activate your key (once per key)
...
def handle_genkey(chat_id, user_id, args):
    ...
    send_message(chat_id, f"{EMOJI['key']} Premium Key Generated:\n\n`/redeem {key_code}`\n\nValid for: {days} days", parse_mode="Markdown")
...
def handle_gentrialkey(chat_id, user_id, args):
    ...
    send_message(chat_id, f"{EMOJI['gift']} Trial Key Generated:\n\n`/redeem {key_code}`\n\nDuration: {duration_str}\nMax uses: {max_uses}", parse_mode="Markdown")
...
def handle_redeem(chat_id, user_id, args):
    ...
    if already_redeemed:
        send_message(chat_id, f"{EMOJI['repeat']}{EMOJI['cross']} You have already redeemed this key before, Alpha! {EMOJI['cross']}{EMOJI['repeat']}\n\n{EMOJI['info']} Each trial key can only be redeemed once per user. Contact @YOWAI_MO_456 for a new key.")
        conn.close()
        return
    ...
    # Record redemption BEFORE granting access (to prevent race conditions)
    ...
    send_message(chat_id, f"""{EMOJI['gift']}✅ TRIAL ACCESS GRANTED ✅{EMOJI['gift']}

{EMOJI['key']} Key: `{key_code}` {EMOJI['check']}
...
{EMOJI['skull']} Welcome to Zeta, warrior. {EMOJI['skull']}""", parse_mode="Markdown")
...
def main():
    ...
    print(f"\n{EMOJI['check']} Zeta Bot v3.1 is LIVE, Alpha!")
    print(f"{EMOJI['alien']} Owner: @YOWAI_MO_456 (ID: {ADMIN_ID})")
    print(f"{EMOJI['alien']} Anti-Duplicate: ENABLED - Users cannot redeem same key twice")
